...
if (salvarBtn) {
    salvarBtn.addEventListener('click', async () => {
        ...
        linhas.forEach(linha => {
            ...
            if (desc || qtd > 0) { 
                itensDetalhados.push({
                    desc: desc, qtd: qtd, refer: linha.querySelector('.item-refer').value,
                    forn: linha.querySelector('.item-forn').value, valor_uni: linha.querySelector('.item-valor').value,
                    valor_final: linha.querySelector('.item-final').value
                });
            }
        });
...
if (imprimirPdfBtn) {
    imprimirPdfBtn.addEventListener('click', () => {
        // Oculta os botões do sistema para não saírem na impressão
        ...
        window.print(); // Abre a tela nativa do celular/PC de salvar em PDF
        
        // Traz os botões de volta após 1 segundo
        setTimeout(() => {
